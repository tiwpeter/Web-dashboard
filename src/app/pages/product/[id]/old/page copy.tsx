...
const ProductId = () => {
  ...
  const [product, setProduct] = useState({
    ...
    stock: "",
    product_image: "",
    category: "", // เพิ่ม category field
  });
  const [image, setImage] = useState(null); // สถานะสำหรับเก็บไฟล์รูปภาพที่ผู้ใช้เลือก
  const [categories, setCategories] = useState([]); // สถานะสำหรับเก็บรายการ categories
  ...
  useEffect(() => {
    ...
    // ดึงรายการ categories สำหรับให้เลือก
    const fetchCategories = async () => {
      try {
        const response = await fetch("http://localhost:3001/api/type/category");
        if (!response.ok) {
          throw new Error("Failed to fetch categories");
        }
        const data = await response.json();
        setCategories(data); // บันทึกรายการ categories
      } catch (error) {
    ...
    fetchCategories(); // เรียกฟังก์ชันดึง categories
  ...
  const handleSave = async () => {
    ...
    formData.append("category", product.category); // เพิ่ม category ในการส่งข้อมูล
